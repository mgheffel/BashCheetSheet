		...
		for b in range(1,8-a):
			if all_data[-1*b]!='na':
				...
				if read1_chro in chr_order and read2_chro in chr_order:
					if [str(read1_chro),str(read1_pos),str(read2_chro),str(read2_pos)]!=past_contact:
						if chr_order.index(read1_chro)<chr_order.index(read2_chro):
							...
							rfh.write(read1_chro+'\t'+str(read1_pos)+'\t'+read2_chro+'\t'+str(read2_pos)+'\t'+read1_st+'\t'+read2_st+'\n')
						if chr_order.index(read1_chro)>chr_order.index(read2_chro):
							#rfh.write('_'.join(fn.split('_')[:3])+'\t'+read2_chro+'\t'+str(read2_pos)+'\t'+read1_chro+'\t'+str(read1_pos)+'\t'+read2_st+'\t'+read1_st+'\n')
							rfh.write(read2_chro+'\t'+str(read2_pos)+'\t'+read1_chro+'\t'+str(read1_pos)+'\t'+read2_st+'\t'+read1_st+'\n')
						if chr_order.index(read1_chro)==chr_order.index(read2_chro):
							...
							if read1_pos>read2_pos and read1_pos-read2_pos>=1000:
								#rfh.write('_'.join(fn.split('_')[:3])+'\t'+read2_chro+'\t'+str(read2_pos)+'\t'+read1_chro+'\t'+str(read1_pos)+'\t'+read2_st+'\t'+read1_st+'\n')
								rfh.write(read2_chro+'\t'+str(read2_pos)+'\t'+read1_chro+'\t'+str(read1_pos)+'\t'+read2_st+'\t'+read1_st+'\n')
					past_contact=[str(read1_chro),str(read1_pos),str(read2_chro),str(read2_pos)]
